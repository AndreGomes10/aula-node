...
// buscar um dado com relacionamento entre as tabelas User e Address (select)
app.get('/users/edit/:id', async (req, res) => {
  const id = req.params.id
  try{
    const user = await User.findOne({ include: Address, where: {id: id}})  // resgatar todos os endereços desse usuario, include: Address
    res.render('useredit', { user: user.get({ plain: true }) })  // pra exibir os dados da forma correta
  } catch(error){
    console.log(error);
  }
})
...
app.post('/address/create', async (req, res) => {
  ...
  res.redirect(`/users/edit/${UserId}`)  // o id vem la da requisição
})
